...
function Estadisticas() {
  ...
  const tdStyle = {
    padding: '.6rem .85rem', borderBottom: '1px solid #f1f5f9',
    color: '#334155', verticalAlign: 'middle', fontSize: '.8rem'
  };

  const card = {
    background: '#fff', borderRadius: '12px',
    border: '1px solid #e2e8f0', padding: '1.25rem', marginBottom: '1rem'
  };
  ...
  const cardHeader = (titulo, extra) => (
    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '1.1rem' }}>
      ...
      <span style={{ fontSize: '.88rem', fontWeight: 700, color: '#1a1a2e' }}>{titulo}</span>
      {extra}
    </div>
  );
        ...
        <>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
                <div style={{ position: 'relative', flexShrink: 0 }}>
                  <svg width="130" height="130" viewBox="0 0 36 36" style={{ transform: 'rotate(-90deg)' }}>
                    {total === 0 ? (
                      <circle cx="18" cy="18" r="15.92" fill="transparent" stroke="#e2e8f0" strokeWidth="5"/>
                    ) : (
                  ...
                  <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', textAlign: 'center' }}>
                    <div style={{ fontSize: '1.2rem', fontWeight: 800, color: '#1a1a2e' }}>{total}</div>
                ...
                <div style={{ flex: 1 }}>
                  {donutData.map((seg, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '.6rem' }}>
                      ...
                      <span style={{ fontSize: '.75rem', color: '#475569', flex: 1 }}>{seg.label}</span>
            ...
            <div style={card}>
              ...
              {Object.entries(estados).map(([label, val]) => (
                <div key={label} style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '.9rem' }}>
                  <div style={{ fontSize: '.74rem', color: '#475569', width: '130px', flexShrink: 0, fontWeight: 500 }}>{label}</div>
          ...
          <div style={{ ...card, marginBottom: '1rem' }}>
                ...
                {filtros.map(f => (
                  <button key={f.value} onClick={() => setFiltroLinea(f.value)}
                    style={{
                      ...
                      background: filtroLinea === f.value ? '#0492C2' : '#f1f5f9',
                      color: filtroLinea === f.value ? '#fff' : '#64748b',
                      transition: 'all .2s'
                    }}>
                    {f.label}
                  </button>
            ...
            <ResponsiveContainer width="100%" height={220}>
              <LineChart data={datosLinea} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
                ...
                <Tooltip
                  contentStyle={{ borderRadius: '8px', border: '1px solid #e2e8f0', fontSize: '.78rem' }}
            ...
            {tiposOrdenados.length === 0 ? (
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: '.75rem' }}>
                {tiposOrdenados.map(([tipo, count]) => (
                  <div key={tipo} style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '10px', padding: '.85rem', textAlign: 'center' }}>
                    <div style={{ fontSize: '.72rem', color: '#64748b', marginBottom: '.3rem', fontWeight: 500 }}>{tipo}</div>
                    ...
                    <div style={{ fontSize: '.68rem', color: '#94a3b8', marginTop: '.3rem' }}>{pct(count)}% del total</div>
                  </div>
                ))}
              </div>
            )}
          ...
          <div style={{ background: '#fff', borderRadius: '12px', border: '1px solid #e2e8f0', overflow: 'hidden' }}>
            <div style={{ padding: '1.25rem 1.25rem .75rem', display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <span style={{ marginLeft: 'auto', fontSize: '.72rem', color: '#94a3b8', background: '#f1f5f9', padding: '2px 9px', borderRadius: '20px' }}>últimos 5</span>
            </div>
                ...
                <tbody>
                  {dispositivos.slice(0, 5).map((d, i) => (
                    <tr key={d.id} style={{ background: i % 2 === 0 ? '#fff' : '#f8fafc' }}>
                      <td style={tdStyle}><div style={{ fontWeight: 600, color: '#1a1a2e', fontSize: '.82rem' }}>{d.nombre}</div></td>
